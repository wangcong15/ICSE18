...
def replace_sizet(file_name):
	file_object = open(file_name)
	all_the_text = file_object.read()
	all_the_text = all_the_text.replace("size_t", "unsigned int")
	file_object.close()
...
# 得到new语句的位置
def find_new(new_cursor):
	global parent_stack
	parent_stack.append(new_cursor)
	result = []
	if new_cursor.kind == clang.cindex.CursorKind.CXX_NEW_EXPR and new_cursor.type.kind == clang.cindex.TypeKind.POINTER:
	...
	for new_cursor_stmt in new_cursor_stmts:
		# temp_void = False
		result += find_new(new_cursor_stmt)
	parent_stack.pop()
	return result

# 获得该函数中new语句的参数
def get_new_param(func_cursor):
	new_array = find_new(func_cursor)
	return_result = []
	for new_a in new_array:
		temp_result = [new_a[0], "assert("+ new_a[1] +" < 1048576);\n"]
...
def get_malloc_param(func_cursor):
	...
	for malloc_a in malloc_array:
		temp_result = [malloc_a[0], "assert("+ malloc_a[1] +" < 1048576);\n"]
		return_result.append(temp_result)
	return return_result
...
if __name__ == "__main__":
	...
	file_parse_error = 0
	# 处理文件夹
	handle_dir("/home/wangcong/Documents/ETAPS/training_sets/training_set_789")
	# handle_file("/home/ubuntu/Document/ETAPS/training_set_789/CWE789_Uncontrolled_Mem_Alloc__new_char_connect_socket_01.c")
	print "-------------------"
